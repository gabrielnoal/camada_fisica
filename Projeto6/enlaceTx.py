...
class TX(object):
    ...
    def createHEAD(self,CRC16, msg_type, payload, packageNum, packageTotal , error8, packageExpected):
        CRC16 = (CRC16).to_bytes(2, byteorder="big")
        ...
        headSize = (self.headSize - (len(CRC16) + len(packageNum) + len(packageTotal) + len(error8) + len(packageExpected) + len(msg_type) + len(payloadSize_bytes) + len(overHead_bytes))) #É o tamanho complementar do head
        head_bytes = (0).to_bytes(headSize, byteorder="big")
        head_bytes += CRC16 + packageNum + packageTotal + error8 + packageExpected + msg_type + overHead_bytes + payloadSize_bytes
        #print("Tamanho do head: {}".format(len(head_bytes)))
        return head_bytes
